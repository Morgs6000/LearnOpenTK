...
public class Game : IDisposable
{
    ...
    public void Init()
    {
        ...
        ResourceManager.LoadShader("src/particle.vs", "src/particle.fs", null, "particle");
        ResourceManager.LoadShader("src/post_processing.vs", "src/post_processing.fs", null, "postprocessing");
        ...
        Effects = new PostProcessor(ResourceManager.GetShader("postprocessing"), (int)Width, (int)Height);
    ...
    public void UpdatePowerUps(float dt)
    {
        foreach (PowerUp powerUp in PowerUps)
        {
            ...
            {
                powerUp.Duraction -= dt;

                if (powerUp.Duraction <= 0.0f)
                {
                    ...
                    if (powerUp.Type == "sticky")
                    {
                        ...
                        {
                            // somente reseta se nenhum outro PowerUp do tipo sticky estiver ativo
                            Ball.Sticky = false;
                            Player.Color = new Vector3(1.0f);
                        }
                    }
