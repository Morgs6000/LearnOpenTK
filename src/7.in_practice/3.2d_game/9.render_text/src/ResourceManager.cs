...
public class ResourceManager
{
    ...
    private static Shader LoadShaderFromFile(string vShaderFile, string fShaderFile, string? gShaderFile = null)
    {
        ...
        try
        {
            vertexCode = RemoveComments(File.ReadAllText(vShaderFile));
            fragmentCode = RemoveComments(File.ReadAllText(fShaderFile));

            if (gShaderFile != null)
            {
                geometryCode = RemoveComments(File.ReadAllText(gShaderFile));
            }
        }
        catch (Exception)
        {
