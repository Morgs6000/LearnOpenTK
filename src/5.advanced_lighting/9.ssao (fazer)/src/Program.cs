...
public class Program : GameWindow
{
    ...
    private float OurLerp(float a, float b, float f)
    {
        return a + f * (b - a);
    }
    ...
    private Shader _shaderSSAO;
    private Shader _shaderSSAOBlur;

    private Model _backpack;

    private uint _gBuffer;
    private uint _gPosition, _gNormal, _gAlbedo;

    private DrawBuffersEnum[] _attachments = new DrawBuffersEnum[3];
    private uint _rboDepth;

    private uint _ssaoFBO, _ssaoBlurFBO;
    private uint _ssaoColorBuffer, _ssaoColorBufferBlur;
    ...
    public Program(GameWindowSettings gameWindowSettings, NativeWindowSettings nativeWindowSettings) : base(gameWindowSettings, nativeWindowSettings)
    {
        ...
        // --------------------------------------------------
        _shaderGeometryPass = new Shader("src/ssao_geometry.vs", "src/ssao_geometry.fs");
        _shaderLightingPass = new Shader("src/ssao.vs", "src/ssao_lighting.fs");
        _shaderSSAO = new Shader("src/ssao.vs", "src/ssao.fs");
        _shaderSSAOBlur = new Shader("src/ssao.vs", "src/ssao_blur.fs");
    ...
    protected override void OnLoad()
    {
        ...
        GL.FramebufferTexture2D(FramebufferTarget.Framebuffer, FramebufferAttachment.ColorAttachment0, TextureTarget.Texture2D, _gAlbedo, 0);
        ...
        // --------------------------------------------------
        GL.GenFramebuffers(1, out _ssaoFBO);
        GL.GenFramebuffers(1, out _ssaoBlurFBO);
        GL.BindFramebuffer(FramebufferTarget.Framebuffer, _ssaoFBO);

        // Buffer de cor SSAO
        GL.GenTextures(1, out _ssaoColorBuffer);
        GL.BindTexture(TextureTarget.Texture2D, _ssaoColorBuffer);

        GL.TexImage2D(TextureTarget.Texture2D, 0, PixelInternalFormat.CompressedRed, SCR_WIDTH, SCR_HEIGHT, 0, PixelFormat.Red, PixelType.Float, IntPtr.Zero);
        ...
        GL.GenTextures(1, out _ssaoColorBufferBlur);
