...
public class Program : GameWindow
{
    ...
    // câmera
    private Camera _camera = new Camera(new Vector3(0.0f, 0.0f, 155.0f));
    ...
    private Shader _asteroidShader;
    ...
    public Program(GameWindowSettings gameWindowSettings, NativeWindowSettings nativeWindowSettings) : base(gameWindowSettings, nativeWindowSettings)
    {
        ...
        // --------------------------------------------------
        _asteroidShader = new Shader("src/asteroids.vs", "src/asteroids.fs");
        _planetShader = new Shader("src/planet.vs", "src/planet.fs");
    ...
    protected override void OnLoad()
    {
        ...
        unsafe
        {
            GL.BufferData(BufferTarget.ArrayBuffer, _amount * sizeof(Matrix4), ref _modelMatrices[0], BufferUsageHint.StaticDraw);
        }
    ...
    protected override void OnRenderFrame(FrameEventArgs args)
    {
        ...
        GL.BindTexture(TextureTarget.Texture2D, _rock.textures_loaded[0].id); // nota: também tornamos público (em vez de privado) o vetor textures_loaded da classe model.

        for (int i = 0; i < _rock.meshes.Count(); i++)
        {
            GL.BindVertexArray(_rock.meshes[i].VAO);
            GL.DrawElementsInstanced(PrimitiveType.Triangles, _rock.meshes[i].indices.Count(), DrawElementsType.UnsignedInt, 0, _amount);
            GL.BindVertexArray(0);
        }
